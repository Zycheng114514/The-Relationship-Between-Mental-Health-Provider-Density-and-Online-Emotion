...
for i in range(0, total_len, CHUNK_SIZE):
    end_index = min(i + CHUNK_SIZE, total_len)
    ...
    for batch in dataloader:
        ...
        chunk_preds.extend(pred_ids)
    
    all_predictions.extend(chunk_preds)
    
    del encoded_dict, input_ids, attention_masks, dataset, dataloader
    torch.cuda.empty_cache()
    gc.collect()

df['is_negative'] = all_predictions
